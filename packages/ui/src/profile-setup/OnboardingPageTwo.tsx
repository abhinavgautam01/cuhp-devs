...
interface OnboardingTwoProps {
  onFinish?: (data: OnboardingStepTwoData) => void | Promise<void>;
  onSkip?: () => void | Promise<void>;
}

export default function OnboardingTwo({ onFinish, onSkip }: OnboardingTwoProps) {
  ...
  const handleNext = async () => {
    ...
    try {
      if (onFinish) {
        await onFinish({ interests: interestsArray });
      } else {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#101322] text-slate-100 font-display relative overflow-hidden">
      ...
      <main className="grow w-full max-w-6xl mx-auto px-6 py-12 relative z-10">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-10"
        >
          <h1 className="text-4xl font-bold mb-3 tracking-tight">What sparks your curiosity?</h1>
          <p className="text-slate-400 max-w-2xl">
            Select the topics you want to see in your daily feed. We&apos;ll tailor your coding
            challenges and learning paths based on these choices.
          </p>
        </motion.div>
          ...
          {Interests.map((interest) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-sm text-slate-400 leading-snug">{interest.description}</p>
              </motion.div>
            );
          })}
        </div>
      ...
      <footer className="sticky bottom-0 w-full bg-[#101322]/80 backdrop-blur-md border-t border-white/10 p-6 z-20">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <button
            ...
            className="text-slate-400 hover:text-white transition-colors font-medium px-4 py-2 disabled:opacity-50"
          >
            Skip for Now
          </button>
