...
export const ProblemInterface: React.FC<ProblemInterfaceProps> = ({
  ...
  const [language, setLanguage] = useState(
    Object.keys(problem.defaultCode)[0] || "Python",
  );
  const [code, setCode] = useState(
    problem.defaultCode[language] ?? "# No code available for this language.",
  );
  ...
  const handleLanguageChange = (newLang: string) => {
    setLanguage(newLang);
    setCode(
      problem.defaultCode[newLang] ?? "# No code available for this language.",
    );
  };
  const runCode = async () => {
    try {
        ...
        <div className="flex-1 flex flex-col bg-[#272822]">
          ...
          <div className="h-14 border-t border-primary-custom/10 bg-background/80 backdrop-blur-md flex items-center justify-between px-6 shrink-0">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={runCode}
                className="px-5 py-2 bg-background/60 hover:bg-background/80 text-slate-200 rounded-lg text-xs font-bold transition-all border border-primary-custom/10"
              >
